...
$resultado=$objSolicitud->consultarSolicitudResueltasPorRol('3');
...
<div class="Atras">
  ...
  <div class="Mensaje2">
    <input class="form-control" type="text" value="Funcionario: <?php echo $nombre->petNombre." ".$nombre->petApellido ?>" aria-label="Disabled input example"
      disabled readonly>
  </div>
  <div class="Mensaje3">
    <input class="form-control" type="text" value="Identificacion: <?php echo $varsession ?>" aria-label="Disabled input example" disabled
      readonly>
  </div>
  ...
  while ($solicitud = $resultado->fetch_object())
  {
	?>
      <tr>
        ...
        <td>
        <?php
          echo "<form name='sdffg' method='POST' action='motivoSolicitudCoordinador.php'>";
...
?>
       ...
       <td>
        ...
        echo "<form name='sdffg' method='POST' action='respuestaSolicitudCoordinador.php'>";
        echo "<input type='hidden' name='radicado' value='$sol'>";
...
?>
        ...
        <td><?php  echo  $solicitud->	petNombre." ".$solicitud->petApellido ?></td>
      </tr>
      <?php
  }
  ?>
  ...
  <div class="SinResolver">
    <div class="d-grid gap-2 d-md-block">
      <a class="btn btn-danger" href="correspondenciaSinResolverC.php" role="button">Sin resolver</a>
    </div></div>
...
if($x==1){
 ...
 swal.fire('Solicitudes que vencen el dia de hoy:','<?php $Conn = new mysqli("localhost","root","","proyecto");
   ...
   tipo_Correspondencia.idTipoCorrespondencia WHERE solicitud.sdDestinatario = 3 and DATE_ADD(solicitud.sdFechaenvio, INTERVAL tipo_correspondencia.tcTiempoRespuesta DAY)
      = CURDATE()";
